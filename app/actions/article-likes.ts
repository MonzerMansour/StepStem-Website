...
export async function initializeArticleLikes(articles: Record<string, number>): Promise<void> {
  try {
    // Check if Redis is properly connected by doing a simple ping
    await redis.ping()
    ...
    for (const [articleId, initialCount] of Object.entries(articles)) {
      try {
        ...
        console.error(`Error initializing likes for article ${articleId}:`, articleError)
    ...
    console.error("Error initializing article likes - Redis connection failed:", error)
    // Don't throw the error, just log it so the app continues to work
    // The like functionality will gracefully degrade
  }
}
